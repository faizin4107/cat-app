...
const COLUMNS = [
...

  {
    ...
    Cell: (row) => {
      ...
      const data = {
        'peraturan': row?.cell?.row.original
      }
      return (
        <div className="flex space-x-3 rtl:space-x-reverse">
          <Tooltip content="Edit" placement="top" arrow animation="shift-away">
            <Link className="action-btn" to={`/edit-peraturan/${row?.cell?.row.original.id}`} state={[propsData, data]}><Icon icon="heroicons:pencil-square" /></Link>
          ...
          >
            ...
            <button className="action-btn" type="button" onClick={() => handleDelete(row?.cell?.row.original.id, 'peraturan')}>
...
const PeraturanTable = ({ title = "Pengaturan Tes" }) => {
  ...
  const location = useLocation();
  const propsData = location.state;
  console.log('propsdata peraturan', propsData);
  // const data = [];
  useEffect(() => {
    // console.log(getKelolaTest());
    async function fetchData() {
      // You can await here
      const response = await getData(`/peraturan/${params.id}`);
    ...
    <>
      <Card>
        <div className="md:flex justify-between items-center mb-6">
          <Breadcrumbschild menu={{ currentMenu: 'Kelola Sesi', path: `/sesi/${params.id}`, childMenu: 'Peraturan', state: propsData }} />
          <Link className="btn-primary btn-sm btn btn inline-flex justify-center" to={`/create-peraturan/${params.id}`} state={propsData}>
